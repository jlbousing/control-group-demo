      ...
      .getConciliationInquiryBySupplier(50,0,data.supplierId,data.startDate,data.endDate)
        .subscribe((response: IConciliationData[]) => {
          ...
          this.conciliations.forEach((item: IConciliationData) => {
            if(typeof item.productionData.quantity === "number") {
              console.log("bandera 1");
              datasets[0].data.push(item.productionData.quantity);
            }
            else if(typeof item.productionData.quantity === "string"){
              console.log("bandera 2");
              datasets[0].data.push(parseInt(item.productionData.quantity));
            }
          });
